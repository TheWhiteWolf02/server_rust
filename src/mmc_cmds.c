...
char *do_rpmb_read_block_if(int nargs, char **argv, unsigned char **out_mac)
{
	int i, ret, dev_fd = -1;
	uint16_t addr;
	/*
	...
	if (nargs == 6)
	{
		...
		int source_len = sizeof(*frame_out) - offsetof(struct rpmb_frame, data);
		unsigned char *source = (unsigned char *)malloc(source_len * sizeof(unsigned char *));
		memcpy(source, &frame_out->data, source_len);
		if (debug)
		{
			// printf("hmac source :");
	...
	/* Read blocks and copy content to output string */
	char *out = (char *)malloc(256 * blocks_cnt);
	for (int i = 0; i < blocks_cnt; i++)
	{
		struct rpmb_frame *frame_out = &frame_out_p[i];
		for (int j = 0; j < 256; j++)
		{
			out[j] = frame_out->data[j];
		}
		// printf("nonce: ");
		//     for (i = 0; i < 16; i++) {
		//         printf("%u", (&frame_out_p[0])->nonce[i]);
		//     }
		//     printf("\n");
	}
	for (int i = 0; i < 32; i++)
...
int do_rpmb_write_block_if(int nargs, char **argv)
{
	int ret, dev_fd;
	char *data;
	unsigned char key[32];
	...
	printf("[INFO] writing to addr: %lu\n", addr);
	frame_in.addr = htobe16(addr);
	...
	else if (datalen != sizeof(frame_in.data))
	{
		printf("Data must be %lu bytes length, but we read only %d, exit\n",
			   (unsigned long)sizeof(frame_in.data),
			   datalen);
	...
	if (debug)
	{
		printf("running hmac sha\n");
		fflush(stdout);
	}
	u_int8_t *key_mac = (u_int8_t *)malloc(sizeof(u_int8_t) * 32);
	hmac_sha256(
		key_p, 32,
		frame_in.data, sizeof(frame_in) - offsetof(struct rpmb_frame, data),
		key_mac, 32);
	...
	if (debug)
	{
		printf("write_counter %u, addr %u, block_count %u, result %u, req_resp %u\n", frame_in.write_counter, frame_in.addr, frame_in.block_count, frame_in.result, frame_in.req_resp);
		printf("hmac source: \n");
		for (int i = 0; i < sizeof(frame_in) - offsetof(struct rpmb_frame, data); i++)
		{
			printf("%u ", frame_in.data[i]);
		}
		printf("\n");
	}
	...
	if (debug)
	{
		...
		for (int i = 0; i < 16; i++)
		{
			printf("%u ", frame_out.nonce[i]);
		}
		printf("\n");
		printf("frame out hmac ");
		for (int i = 0; i < 32; i++)
		{
			printf("%u ", frame_out.key_mac[i]);
		}
		printf("\n");
	}
	close(dev_fd);
	printf("RPMB write complete\n");
	return ret;
}
